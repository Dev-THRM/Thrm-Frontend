...
export default function AdminFounders() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!image) {
      setStatus({ type: "error", message: "Please upload a founder profile image." });
    ...
    try {
      ...
      if (response.ok && result.success) {
        setStatus({ type: "success", message: "Founder profile published successfully! Redirecting..." });
        ...
        setTimeout(() => {
          navigate(`/founders/${targetSlug}`);
        }, 1500);
      } else {
        setStatus({ type: "error", message: result.error || "Failed to publish founder profile." });
      }
    } catch (error) {
      console.error("Founder upload error:", error);
    ...
    <div className="min-h-screen bg-[#02040a] text-white pt-32 px-6 flex justify-center pb-20">
      <div className="w-full max-w-4xl bg-[#060b18]/80 border border-white/10 p-8 md:p-10 rounded-4xl backdrop-blur-xl shadow-2xl relative">
        ...
        <div className="mb-8 border-b border-white/10 pb-6">
          <h2 className="text-3xl font-black tracking-tight">Add a New Founder Episode</h2>
          <p className="text-white/50 mt-2">Publish a new interview or story profile for the Founders Series.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-white/40 ml-1 mt-1">Prefer 3:4 aspect ratio portrait images.</p>
              </div>
          ...
          >
            {isSubmitting ? "Publishing Founder Profile..." : "Publish Founder Profile"}
          </button>
        </form>
      </div>
    </div>
